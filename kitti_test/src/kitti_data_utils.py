...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, fixpath=None):
    ...
    def read_calib_file(self, filepath, fixpath=None):

        if filepath=='fix':
            filepath = '/home/vip/config/'
            calib_info = read_fixed_frame_calib(filepath)
        else:
...
def read_fixed_frame_calib(filepath):



    cam2cam_file = open(os.path.join(filepath, "calib_cam_to_cam.txt" ), "r")
    velo2cam_file = open(os.path.join(filepath, "calib_velo_to_cam.txt" ), "r")

    for line in cam2cam_file:
        if line.strip().split(":")[0] == "R_rect_02":
            obj  = line.split(" ")[1: ]
            R0 = np.array(obj, dtype=np.float32)
        elif line.strip().split(":")[0] == "P_rect_02":
            obj  = line.split(" ")[1: ]
            P2 = np.array(obj, dtype=np.float32)
        elif line.strip().split(":")[0] == "P_rect_03":
            obj  = line.split(" ")[1: ]
            P3 = np.array(obj, dtype=np.float32)
